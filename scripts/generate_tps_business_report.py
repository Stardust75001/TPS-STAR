...
def footer(canvas: Canvas, doc):
    """
    Footer avec numéro de page + date de génération,
...
def load_optional_metrics_csv(filename: str):
    ...
    Retourne un dict {metric: value} ou {} si absent/incompatible.
    """
...
def generate_health_chart(df):
    """
    Génère un bar chart sur la santé des services
    (OK / INVALID / MISSING / etc.).
    """
...
if not summary_lines:
    summary_text = (
        "Données business détaillées non encore branchées (Shopify / GA4 / Meta). "
        "Le rapport utilise pour l’instant principalement l’état des intégrations techniques."
    )
else:
...
dq_rows = [
    ...
    ["Historique erreurs système", "À enrichir avec des logs / stats supplémentaires."],
]
...
if not analysis_lines:
    analysis_lines.append(
        ...
        "Brancher Shopify / GA4 / Meta pour enrichir ce rapport."
    )
